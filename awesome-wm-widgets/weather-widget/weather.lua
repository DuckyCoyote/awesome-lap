...
local WIDGET_DIR = HOME_DIR ..
                       '/.config/awesome/awesome-wm-widgets/weather-widget'
...
local LANG = gears.filesystem.file_readable(
                 WIDGET_DIR .. "/" .. "locale/" .. SYS_LANG .. ".lua") and
                 SYS_LANG or "en"
...
local function uvi_index_color(uvi)
    ...
    return '<span weight="bold" foreground="' .. color .. '">' .. uvi ..
               '</span>'
end
...
local function worker(user_args)
    ...
    local owm_one_cal_api =
        ('https://api.openweathermap.org/data/2.5/onecall' .. '?lat=' ..
            ...
            (show_daily_forecast == false and ',daily' or '') .. '&lang=' ..
            LANG)
    ...
    weather_widget = wibox.widget {
        ...
        is_ok = function(self, is_ok)
            if is_ok then
                ...
                self:get_children_by_id('icon')[1]:emit_signal(
    ...
    local current_weather_widget = wibox.widget {
        ...
        update = function(self, weather)
            self:get_children_by_id('icon')[1]:set_image(ICONS_DIR ..
                                                             icon_map[weather.weather[1]
                                                                 .icon] ..
                                                             icons_extension)
            self:get_children_by_id('temp')[1]:set_text(
                gen_temperature_str(weather.temp, ' %.0f', false, units))
            self:get_children_by_id('feels_like_temp')[1]:set_text(
                LCLE.feels_like ..
                    gen_temperature_str(weather.feels_like, ' %.0f', false,
                                        units))
            self:get_children_by_id('description')[1]:set_text(
                weather.weather[1].description)
            self:get_children_by_id('wind')[1]:set_markup(
                LCLE.wind .. '<b>' .. weather.wind_speed .. 'm/s (' ..
                    to_direction(weather.wind_deg) .. ')</b>')
            self:get_children_by_id('humidity')[1]:set_markup(LCLE.humidity ..
                                                                  '<b>' ..
                                                                  weather.humidity ..
                                                                  '%</b>')
            self:get_children_by_id('uv')[1]:set_markup(LCLE.uv ..
                                                            uvi_index_color(
                                                                weather.uvi))
        end
    }
        ...
        update = function(self, forecast, timezone_offset)
            ...
            for i, day in ipairs(forecast) do
                ...
                local day_forecast = wibox.widget {
                    {
                        text = os.date('%a', tonumber(day.dt) +
                                           tonumber(timezone_offset)),
                        align = 'center',
                        font = font_name .. ' 9',
                        widget = wibox.widget.textbox
                    },
                    ...
                    {
                        {
                            text = gen_temperature_str(day.temp.day, '%.0f',
                                                       false, units),
                            align = 'center',
                            font = font_name .. ' 9',
                            widget = wibox.widget.textbox
                        ...
                        {
                            text = gen_temperature_str(day.temp.night, '%.0f',
                                                       false, units),
                            align = 'center',
                            font = font_name .. ' 9',
                            widget = wibox.widget.textbox
    ...
    local hourly_forecast_widget = {
        ...
        update = function(self, hourly)
            ...
            for i, hour in ipairs(hourly) do
                ...
                if (i - 1) % 5 == 0 then
                    table.insert(hours_below, wibox.widget {
                        text = os.date(time_format_12h and '%I%p' or '%H:00',
                                       tonumber(hour.dt)),
                        align = 'center',
                        font = font_name .. ' 9',
                        widget = wibox.widget.textbox
            ...
            hourly_forecast_graph:set_max_value(math.max(max_temp,
                                                         math.abs(min_temp)))
            hourly_forecast_graph:set_min_value(
                min_temp > 0 and min_temp * 0.7 or 0) -- move graph a bit up

            hourly_forecast_negative_graph:set_max_value(math.abs(min_temp))
            hourly_forecast_negative_graph:set_min_value(max_temp < 0 and
                                                             math.abs(max_temp) *
                                                             0.7 or 0)
            ...
            elseif max_temp < 0 then
                ...
                table.insert(self, wibox.widget {
                    ...
                    {temp_below, valign = 'top', widget = wibox.container.place},
                    id = 'graph',
                    layout = wibox.layout.stack
                })
    ...
    local function update_widget(widget, stdout, stderr)
        if stderr ~= '' then
            if not warning_shown then
                if (stderr ~= 'curl: (52) Empty reply from server' and stderr ~=
                    'curl: (28) Failed to connect to api.openweathermap.org port 443: Connection timed out' and
                    stderr:find(
                        '^curl: %(18%) transfer closed with %d+ bytes remaining to read$') ~=
                    nil) then show_warning(stderr) end
                warning_shown = true
                widget:is_ok(false)
        ...
        widget:set_image(
            ICONS_DIR .. icon_map[result.current.weather[1].icon] ..
                icons_extension)
        widget:set_text(gen_temperature_str(result.current.temp, '%.0f',
                                            both_units_widget, units))
        ...
        weather_popup:setup({
            {final_widget, margins = 10, widget = wibox.container.margin},
    ...
    watch(string.format(GET_FORECAST_CMD, owm_one_cal_api), timeout, -- API limit is 1k req/day; day has 1440 min; every 2 min is good
